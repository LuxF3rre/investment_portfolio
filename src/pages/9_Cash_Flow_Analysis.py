...
_MODE_HELP: dict[str, str] = {
    "IRR": (
        ...
        "$$\\text{NPV} = \\sum_{t=0}^{N} "
        "\\frac{C_t}{(1 + r)^{\\,t}} = 0"
        "\\quad \\Longrightarrow \\quad r = \\text{IRR}$$\n\n"
        "where $C_t$ is the cash flow at period $t$ and $r$ is "
        "the discount rate.\n\n"
        "**No parameters** — IRR is fully determined by the "
        "cash flows."
    ),
    "MIRR": (
        ...
        "$$\\text{MIRR} = "
        "\\left(\\frac{\\text{FV}_{\\text{pos}}}"
        "{\\lvert\\,\\text{PV}_{\\text{neg}}\\rvert}"
        "\\right)^{1/N} - 1$$\n\n"
        "$$\\text{FV}_{\\text{pos}} = "
        "\\sum_{C_t > 0} C_t\\,(1+r_{\\text{reinv}})^{N-t}"
        "\\;,\\qquad "
        "\\text{PV}_{\\text{neg}} = "
        "\\sum_{C_t < 0} \\frac{C_t}"
        "{(1+r_{\\text{fin}})^{\\,t}}$$\n\n"
        "**Parameters:**\n"
        "- **Finance rate** $r_{\\text{fin}}$ — the cost of "
        "borrowing to cover negative cash flows (e.g. your loan "
        "rate or WACC).\n"
        "- **Reinvestment rate** $r_{\\text{reinv}}$ — the rate "
        "at which positive cash flows are reinvested (e.g. a "
        "savings rate or market return)."
    ),
    "GIRR": (
        ...
        "\\;,\\quad S_0 = C_0"
        "\\;,\\quad S_N = 0$$\n\n"
        "where $r_I$ is the unknown investment rate (= GIRR) "
        "and $r_F$ is the known finance rate.\n\n"
        ...
        "**Parameters:**\n"
        "- **Finance rate** $r_F$ — the external rate applied "
        "when the project balance is non-negative (excess cash "
        "reinvested externally). The solver finds $r_I$ for "
    ...
    "AIRR": (
        ...
        "$$\\bar{r}_{\\text{AIRR}} = r_c + "
        "\\frac{\\text{NPV}(r_c)}{C_0^{\\,\\text{PV}}}$$\n\n"
        "where $r_c$ is the cost of capital and "
        "$C_0^{\\,\\text{PV}}$ is the present value of the "
        "invested capital schedule:\n\n"
        "$$C_0^{\\,\\text{PV}} = \\sum_{t=0}^{N-1} "
        "\\frac{c_t}{(1 + r_c)^{\\,t}}$$\n\n"
        "A project creates value iff "
        "$\\bar{r}_{\\text{AIRR}} > r_c$, which is equivalent "
        "to $\\text{NPV} > 0$.\n\n"
        "**Parameters:**\n"
        "- **Cost of capital** $r_c$ — the opportunity cost / "
        "discount rate used to compute NPV and the present "
        "value of invested capital.\n"
        "- **Depreciation method** — how the capital schedule "
        "$\\{c_t\\}$ is built. *Straight-line* amortises the "
        "initial investment evenly over all periods. "
        "*IRR-implied* uses the first IRR root to derive the "
        "schedule from the project balance equation."
    ),
    "Horizon IRR": (
        ...
        "r = \\text{Horizon IRR}$$\n\n"
        "where $H$ is the horizon period and $TV$ is the "
        "terminal value added at $t = H$.\n\n"
        "**Parameters:**\n"
        "- **Horizon period** $H$ — the period at which the "
        "project is liquidated. Cash flows beyond this are "
        "ignored.\n"
        "- **Terminal value** $TV$ — the residual / salvage / "
        "liquidation value received at the horizon (added to "
        "the last cash flow)."
    ),
    "Pairwise IRR": (
        "Computes the IRR of the **incremental** cash flows "
        "$\\Delta C_t = C_t^A - C_t^B$ between two mutually "
        "exclusive projects.\n\n"
        "$$\\sum_{t=0}^{N} "
        "\\frac{C_t^A - C_t^B}{(1 + r)^{\\,t}} = 0"
        "\\quad \\Longrightarrow \\quad "
        "r = \\text{Pairwise IRR}$$\n\n"
        "The preference is determined by the NPV of the "
        "incremental stream at the cost of capital: if "
        "$\\text{NPV}(\\Delta C) > 0$ at $r_c$, project A "
        "is preferred.\n\n"
        "**Parameters:**\n"
        "- **Cost of capital** $r_c$ — the hurdle rate for the "
        "decision rule."
    ),
}
