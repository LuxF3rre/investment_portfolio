...
class TestFetchMultiHistory:
    ...
    def test_correct_columns(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        close_data = {t: 100 + rng.standard_normal(n).cumsum() for t in tickers}
        mock_df = pd.DataFrame(close_data, index=index)
        multi_col = pd.MultiIndex.from_product([["Close"], tickers])
        mock_raw = pd.DataFrame(mock_df.values, index=index, columns=multi_col)

        def _mock_download(*_args: object, **_kwargs: object) -> pd.DataFrame:
            return mock_raw

        monkeypatch.setattr("investment_portfolio.mpt.yf.download", _mock_download)
        result = fetch_multi_history(tickers=tickers)

        assert list(result.columns) == tickers
        assert len(result) == n

    def test_raises_on_empty_download(self, monkeypatch: pytest.MonkeyPatch) -> None:
        def _mock_download(*_args: object, **_kwargs: object) -> pd.DataFrame:
            return pd.DataFrame()

        monkeypatch.setattr("investment_portfolio.mpt.yf.download", _mock_download)
        with pytest.raises(ValueError, match="no data returned"):
            fetch_multi_history(tickers=["BAD1", "BAD2"])
